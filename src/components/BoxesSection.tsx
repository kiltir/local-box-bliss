  ...
  return <section id="boxes" className="py-[15px]">
      <div className="container-section py-[15px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 gap-8 fade-in max-w-md mx-auto">
          {filteredBoxes.map(box => <BoxCard key={box.id} title={box.baseTitle} price={box.price} description={box.description} image={box.image} items={box.items} theme={box.theme} onClick={() => handleBoxClick(box.id)} />)}
        </div>
      </div>
